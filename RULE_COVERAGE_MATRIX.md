...
| `disabled in-hosted (structural FP)` | The rule can never produce a truthful result inside a hosted cluster because the platform behaviour it checks does not exist there (not a namespace/target mismatch). Disabled in `hosted/tp.yaml`; the control is carried as an SSP statement with a management-cluster compensating control. |
...
| `cluster_version_operator_verify_integrity` | A | disabled (wrong-target) | - | disabled in-hosted (structural FP) | PASS (vacuous) | SSP statement. The CVO runs NO release-image signature verification on HyperShift (confirmed by OTA/HCP engineering) - `verified` is false on every history entry forever. The rule's jq filter `[.status.history[0:-1][].verified]` drops the newest entry, so a never-upgraded hosted cluster passes on an empty set (what round 1 observed on the fresh `hcp-aws`), while any upgraded hosted cluster FAILs permanently (history is immutable). Now disabled in `hosted/tp.yaml`. The CONTROL is not lost: CNTR-OS-000740 / SA-10(1) map to a rule SET, and the signature requirement is carried by `reject_unsigned_images_by_default` (`/etc/containers/policy.json` default `reject`) in `ocp4-stig-node` / `ocp4-high-node` - the supported in-hosted profile surface - plus `cluster_version_operator_exists` and the registry allowlist rules. Deliver the policy.json remediation through `NodePool.spec.config`. Compensating control: the management cluster's own CVO verifies its releases and the HyperShift Operator/CPO controls which release image the hosted control plane runs. Inspect with: `oc get clusterversion version -o jsonpath='{range .status.history[*]}{.version} verified={.verified}{"\n"}{end}'` |
| `configure_network_policies` | A | auto N/A | - | covered in-hosted | PASS | Hosted scan |
| `configure_network_policies_namespaces` | A | auto N/A | - | covered in-hosted | FAIL | Hosted scan |
